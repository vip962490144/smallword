...
def send_world_redpacket(login_token):
    ...
    data_list = {"type": 3, "user_num": "100", "total_money": 3000, "memo": "新年快乐"}
...
if __name__ == '__main__':
    ...
    for i in range(100):
        ...
        # user_info = UserBase().get_user_info(token, "nickname,ticket")
    ...
    for i in li:
        ...
        # 获得领取红包的金额
        # receive_redpacket_money = i.get()["packet"]["current_user_packet_money"]
        ...
        # user_info_after.append(receive_redpacket_ticket)

    # user_info_dict_after = []
    # for var in list_token:
    #     user_info = UserBase().get_user_info(token, "nickname,ticket")
    #     user_info_dict_after.append(user_info)
    #
    # b = 0
    # for user in range(len(user_info_dict)):
    #     for user_after in range(len(user_info_dict_after)):
    #         user_num = float(user_info_dict[b]["ticket"]) - float(user_info_dict_after[b]["ticket"])
    #         sex = user_num - user_info_after[b]
    #         list_num = [-1, 0, 1]
    #         if sex in list_num:
    #             print("相等")
    #             continue

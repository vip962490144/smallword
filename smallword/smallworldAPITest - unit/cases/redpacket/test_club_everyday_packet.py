...
class ClubEverydayPacket(unittest.TestCase):
    """
    俱乐部每日红包
    """
    @classmethod
    ...
    def test_case_01(self):
        ...
        try:
            self.assertEqual(int(a_user_info_after_diamond) + consumption_diamond,
                             int(a_user_info_befoer_diamond), msg="钻石")
            self.assertIn(a_user_rank_num - int(a_user_info_after_rank), verdict, msg="实力榜")
            self.assertIn(b_user_rank_num - int(b_user_info_after_rank), verdict, msg="魅力榜")
            self.assertIn(ticket_num, verdict, msg="礼券")
            self.assertIn(guess_diamond_num, verdict, msg="奖池")

...
class PartyPacket(unittest.TestCase):
    """
    聚会红包
    """
    @classmethod
    ...
    def test_case_01(self):
        ...
        party_id = PartyBase().get_party_id(a_login_token, data_list)
        ...
        actual = PartyBase().apply_for_party(login_token, party_id)
        # print(actual.json()["id"])
        apply_id = actual.json()["id"]
        actual = LoginBase().login(do_config("username", "A"))
        a_login_token = actual.json()["token"]
        actual = PartyBase().handle_apply(a_login_token, apply_id)
        # print(actual.json())

        data_list2 = {"appoint_time": 0}

        actual = PartyBase().update_party_info(a_login_token, data_list2, party_id)
        # print(actual.json())
        ...
        # 断言，判断前后的差值
        try:
            self.assertEqual(int(a_user_info_after_diamond) + int(data_list["packet_money"]) * 13,
